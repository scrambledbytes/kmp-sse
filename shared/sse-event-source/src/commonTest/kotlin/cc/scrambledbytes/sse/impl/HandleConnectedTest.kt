...
class HandleConnectedTest {
    ...
    fun testWhenOkOpenConnection() = runBlocking {
        ...
        source.state.test {
            ...
            assertEquals(SseEventSource.State(statusCode = 200, ready = OPEN, isFailed = false), awaitItem())
            expectNoEvents()
        }
    }
    ...
    fun testWhenRetryOpenConnection() = runBlocking {
        ...
        source.state.test {
            assertEquals(SseEventSource.State(), awaitItem())
            assertEquals(SseEventSource.State(statusCode = 201, ready = CLOSED, isFailed = false), awaitItem())
            assertEquals(SseEventSource.State(statusCode = 201, ready = CONNECTING, isFailed = false), awaitItem())
            expectNoEvents()
        }
    }
    ...
    fun testWhenFailClose() = runBlocking {
        ...
        source.state.test {
            assertEquals(SseEventSource.State(), awaitItem())
            assertEquals(SseEventSource.State(statusCode = 204, ready = CLOSED, isFailed = true), awaitItem())
            expectNoEvents()
        }
